...
# Configurações da comunicação serial com ESP32
SERIAL_PORT = "COM3"  # Altere para a porta do seu ESP32
BAUD_RATE = 9600
...
def setup_serial_connection():
    """Configura a conexão serial com o ESP32"""
    try:
        ser = serial.Serial(SERIAL_PORT, BAUD_RATE, timeout=1)
        time.sleep(2)
        print("Conexão serial estabelecida com ESP32")
        return ser
    except Exception as e:
        print(f"Erro na conexão serial: {e}")
        return None

def control_door(ser, action):
    ...
    try:
        ...
        print(f"Erro no controle da porta: {e}")
        return False
...
def main():
    ...
    try:
        ...
        # 6. Codificar labels (usando nosso próprio encoder)
        ...
        print("Pressione 'o' para abrir a porta manualmente (teste)")
                    ...
                    if w > 20 and h > 20:
                        ...
                        if confidence < 70:
                            ...
                            if not authorized_person_detected:
                                door_result = control_door(ser, "open")
                                if door_result:
